...
static local_id numChildProcs = 0;
...
local_id GetNumChildProcesses(){
	return numChildProcs;
}

local_id CreateAndInitChildProcesses(local_id numChildProcesses, IPCHelper** ipc, const balance_t* balances){
	if (numChildProcesses <1 || numChildProcesses > MAX_NUM_CHILD_PROCESSES)
		return -1;
	local_id numProcesses = numChildProcesses + 1;

	numChildProcs = numChildProcesses;
